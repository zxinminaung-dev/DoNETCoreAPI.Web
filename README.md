# .NET Core API application with Entity Framework Core, Generic Repository, Autofac, Unit Of Work, Microsoft SQL Server, Linq

[Generic repository](https://dotnettutorials.net/lesson/generic-repository-pattern-csharp-mvc)


[Entity Framework Core](https://dotnettutorials.net/lesson/entity-framework-core/) 


[Auto Fac](https://autofac.org/)


[Unit Of Work Pattern](https://dotnettutorials.net/lesson/unit-of-work-csharp-mvc/)


[Microsoft SQL Server with C#](https://dotnettutorials.net/course/ms-sql-server/)


[Linq](https://dotnettutorials.net/course/linq/)


[System.Web.Helpers.Crypto](https://learn.microsoft.com/en-us/dotnet/api/system.web.helpers.crypto?view=aspnet-webpages-3.2

